  ...
  server <- function(input, output, session) {
    ...
    #Initialize Variables
    {
      #Species metadata
      {
        ...
        ###
        Supported_Site_List <- unique(Supported_Species$Site)
        
        Supported_Site_List <- c()
        for (i in 1:length(unique(Supported_Species$Site))) {
          Supported_Site_List <- append(Supported_Site_List, unlist(strsplit(unique(Supported_Species$Site)[i], ",")))
        }
        for(i in 1:length(Supported_Site_List)) {
          if (substr(Supported_Site_List[i], 1,1) == " ") {
            Supported_Site_List[i] <- substr(Supported_Site_List[i], 2, nchar(Supported_Site_List[i]))
          }
        }
        for (i in 1:length(Supported_Site_List)) {
          if (substr(Supported_Site_List[i], nchar(Supported_Site_List[i]), nchar(Supported_Site_List[i])) == " ") {
            Supported_Site_List[i] <- substr(Supported_Site_List[i], 1, nchar(Supported_Site_List[i])-1)
          }
        }
        ###
      }
    ...
    #Update species selection menus
    {
      ...
      observe({
        updateSelectizeInput(session, inputId = "Filter_Species_Site", choices = Supported_Site_List, selected = "")
      })
